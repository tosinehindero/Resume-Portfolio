         ...
         <div>
            {EXPERIENCES.map((experience, index) => (
               <div key={index} className="mb-12 flex flex-wrap lg:justify-center">
                  ...
                  <motion.div whileInView={{opacity:1, x:0}} initial={{opacity:0, x:-100}} transition={{duration:1}} className="w-full lg:w-1/4 lg:px-4">
                     {" "}
                     {/* Add padding for spacing */}
                     <p className="mb-2 text-sm text-neutral-400">{experience.year}</p>
                  </motion.div>
                  <motion.div whileInView={{opacity:1, x:0}} initial={{opacity:0, x:100}} transition={{duration:1}} className="w-full max-w-xl lg:w-3/4 lg:px-4">
                     {" "}
                     {/* Add padding for spacing */}
                     <h6 className="mb-2 font-semibold">
                        {experience.role} -{" "}
                     ...
                     <div className="flex flex-wrap gap-2">
                        {" "}
                        {/* Adjust spacing for technologies */}
                        {experience.technologies.map((tech, index) => (
                           <span
                              key={index}
